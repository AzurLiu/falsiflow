...
EXPECTED_PROJECT_URLS = {
    ...
    "DownstreamRagDemo": "https://github.com/AzurLiu/falsiflow-downstream-rag-eval-demo",
...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    product_metric_article_path = root / "docs" / "launch_articles" / "evidence_gates_for_product_metrics.md"
    benchmark_article_path = root / "docs" / "launch_articles" / "benchmarks_should_fail_builds.md"
    ...
    public_issue_queue_path = root / "docs" / "falsiflow_public_issue_queue.md"
    ...
    mvp_path = root / "docs" / "falsiflow_mvp.md"
    ...
    downstream_rag_smoke_path = root / "examples" / "downstream_rag_eval_smoke"
    downstream_rag_smoke_readme_path = downstream_rag_smoke_path / "README.md"
    downstream_rag_smoke_workflow_path = downstream_rag_smoke_path / ".github" / "workflows" / "falsiflow-rag-eval.yml"
    downstream_rag_smoke_project_path = downstream_rag_smoke_path / "falsiflow_rag_eval" / "project.json"
    downstream_rag_smoke_evidence_path = downstream_rag_smoke_path / "falsiflow_rag_eval" / "evidence.csv"
    downstream_rag_smoke_placeholder_path = downstream_rag_smoke_path / "falsiflow_rag_eval" / "evidence_placeholder_demo.csv"
    downstream_rag_smoke_pass_path = downstream_rag_smoke_path / "falsiflow_rag_eval" / "evidence_pass_demo.csv"
    downstream_rag_smoke_source_path = downstream_rag_smoke_path / "falsiflow_rag_eval" / "source_files" / "rag_eval_raw_export.csv"
    local_llm_fixture_path = root / "examples" / "local_llm_eval_import"
    local_llm_fixture_readme_path = local_llm_fixture_path / "README.md"
    local_llm_fixture_workflow_path = local_llm_fixture_path / ".github" / "workflows" / "falsiflow-local-llm-eval.yml"
    local_llm_fixture_project_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "project.json"
    local_llm_fixture_evidence_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "evidence.csv"
    local_llm_fixture_placeholder_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "evidence_placeholder_demo.csv"
    local_llm_fixture_imported_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "evidence_imported_demo.csv"
    local_llm_fixture_manifest_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "local_model_manifest.json"
    local_llm_fixture_source_path = local_llm_fixture_path / "falsiflow_local_llm_eval" / "source_files" / "local_eval_results.jsonl"
    ...
    if pyproject_path.exists():
        ...
        add("product_metric_article_exists", product_metric_article_path.exists() and product_metric_article_path.stat().st_size > 0, "Product metric launch article exists and is non-empty.", product_metric_article_path)
        add("benchmark_article_exists", benchmark_article_path.exists() and benchmark_article_path.stat().st_size > 0, "Benchmark launch article exists and is non-empty.", benchmark_article_path)
        ...
        downstream_rag_smoke_files = [
            downstream_rag_smoke_readme_path,
            downstream_rag_smoke_workflow_path,
            downstream_rag_smoke_project_path,
            downstream_rag_smoke_evidence_path,
            downstream_rag_smoke_placeholder_path,
            downstream_rag_smoke_pass_path,
            downstream_rag_smoke_source_path,
        ]
        add("downstream_rag_eval_smoke_fixture_exists", all(path.exists() and path.stat().st_size > 0 for path in downstream_rag_smoke_files), "Downstream RAG eval smoke fixture includes README, workflow, project, placeholder, pass, evidence, and source files.", downstream_rag_smoke_path)
        ...
        product_metric_article_text = product_metric_article_path.read_text(encoding="utf-8") if product_metric_article_path.exists() else ""
        benchmark_article_text = benchmark_article_path.read_text(encoding="utf-8") if benchmark_article_path.exists() else ""
        ...
        public_issue_queue_text = public_issue_queue_path.read_text(encoding="utf-8") if public_issue_queue_path.exists() else ""
        ...
        mvp_text = mvp_path.read_text(encoding="utf-8") if mvp_path.exists() else ""
        ...
        downstream_rag_smoke_readme_text = downstream_rag_smoke_readme_path.read_text(encoding="utf-8") if downstream_rag_smoke_readme_path.exists() else ""
        downstream_rag_smoke_workflow_text = downstream_rag_smoke_workflow_path.read_text(encoding="utf-8") if downstream_rag_smoke_workflow_path.exists() else ""
        downstream_rag_smoke_evidence_text = downstream_rag_smoke_evidence_path.read_text(encoding="utf-8") if downstream_rag_smoke_evidence_path.exists() else ""
        downstream_rag_smoke_placeholder_text = downstream_rag_smoke_placeholder_path.read_text(encoding="utf-8") if downstream_rag_smoke_placeholder_path.exists() else ""
        downstream_rag_smoke_pass_text = downstream_rag_smoke_pass_path.read_text(encoding="utf-8") if downstream_rag_smoke_pass_path.exists() else ""
        local_llm_fixture_readme_text = local_llm_fixture_readme_path.read_text(encoding="utf-8") if local_llm_fixture_readme_path.exists() else ""
        local_llm_fixture_workflow_text = local_llm_fixture_workflow_path.read_text(encoding="utf-8") if local_llm_fixture_workflow_path.exists() else ""
        local_llm_fixture_evidence_text = local_llm_fixture_evidence_path.read_text(encoding="utf-8") if local_llm_fixture_evidence_path.exists() else ""
        local_llm_fixture_placeholder_text = local_llm_fixture_placeholder_path.read_text(encoding="utf-8") if local_llm_fixture_placeholder_path.exists() else ""
        local_llm_fixture_imported_text = local_llm_fixture_imported_path.read_text(encoding="utf-8") if local_llm_fixture_imported_path.exists() else ""
        local_llm_fixture_manifest_text = local_llm_fixture_manifest_path.read_text(encoding="utf-8") if local_llm_fixture_manifest_path.exists() else ""
        local_llm_fixture_source_text = local_llm_fixture_source_path.read_text(encoding="utf-8") if local_llm_fixture_source_path.exists() else ""
        ...
        docs_reference_text = "\n".join(
            [
                ...
                roadmap_text,
                adoption_priorities_text,
                launch_plan_text,
                launch_execution_text,
                launch_article_text,
                product_metric_article_text,
                benchmark_article_text,
                architecture_text,
                cli_reference_text,
                data_contract_text,
                adapter_profiles_text,
                local_llm_eval_text,
                mcp_text,
                casebook_check_text,
                demo_pr_playbook_text,
                github_action_examples_text,
                public_issue_queue_text,
                positioning_text,
                public_casebook_text,
                rag_quality_gate_proposal_text,
                pypi_trusted_publishing_text,
                security_posture_text,
                template_authoring_text,
                troubleshooting_text,
                mvp_text,
                public_demo_index_text,
                walkthrough_text,
                makefile_text,
            ]
        )
        maintainer_docs_text = "\n".join(
            [
                readme_text,
                release_text,
                contributing_text,
        ...
        cli_docs_text = "\n".join([readme_text, cli_reference_text, walkthrough_text, makefile_text, troubleshooting_text, mvp_text])
        template_docs_text = "\n".join([template_authoring_text, cli_reference_text, walkthrough_text, contributing_text])
        launch_docs_text = "\n".join(
            [
                readme_text,
                release_text,
        ...
        public_interface_docs_text = "\n".join([cli_reference_text, data_contract_text, architecture_text, positioning_text, walkthrough_text, mvp_text])
        ...
        add("readme_release_commands", all(token in maintainer_docs_text for token in ["release-check", "verify-bundle", "falsiflow selftest"]), "Docs surface release, verification, and selftest commands without forcing maintainer details into the README first screen.", release_path)
        add("readme_community_health_entry", all(path.exists() and path.stat().st_size > 0 for path in [code_of_conduct_path, support_path, roadmap_path]) and all(token in maintainer_docs_text for token in ["CODE_OF_CONDUCT.md", "SUPPORT.md", "ROADMAP.md"]), "Community health entry points exist and are linked from the docs surface.", support_path)
        add("readme_citation_governance_entry", all(path.exists() and path.stat().st_size > 0 for path in [citation_path, governance_path]) and all(token in maintainer_docs_text for token in ["CITATION.cff", "GOVERNANCE.md"]), "Citation and governance entry points exist and are linked from the docs surface.", governance_path)
        add("readme_architecture_entry", "falsiflow_architecture.md" in readme_text and all(token in architecture_text for token in ["Module Map", "Extension Points"]), "README links architecture while the architecture doc carries the module map and extension-point notes.", architecture_path)
        add("readme_data_contract_entry", "falsiflow_data_contract.md" in readme_text and all(token in data_contract_text for token in ["Evidence CSV", "JSON Status Contract", "Integration Guidance"]), "README links the data contract while the data-contract doc carries evidence CSV, JSON status, and integration boundaries.", data_contract_path)
        add("readme_adapter_profiles_entry", "falsiflow_adapter_profiles.md" in readme_text and all(token in docs_reference_text for token in ["vendor-measurement", "instrument-export", "plate-reader", "falsiflow_local_llm_eval.md", "Ollama", "LM Studio", "llama.cpp"]), "README links adapter profiles while docs carry vendor, instrument, plate-reader, and local LLM eval import guidance.", adapter_profiles_path)
        add("readme_mcp_entry", "falsiflow_mcp.md" in readme_text and all(token in mcp_text for token in ["falsiflow mcp --selftest --json", "mcp_selftest_ready", "falsiflow mcp", "mcpServers", "\"command\": \"falsiflow\"", "\"args\": [\"mcp\"]", "no HTTP listener", "do not run a model"]), "README links the local stdio MCP guide while MCP docs carry selftest, client config, and no-network/no-model boundaries.", mcp_path)
        add("readme_template_authoring_entry", "falsiflow_template_authoring.md" in readme_text and all(token in template_authoring_text for token in ["Template Layout", "placeholder demo", "template-release"]), "README links template authoring while docs carry placeholder demo and release flow guidance.", template_authoring_path)
        add("readme_troubleshooting_entry", "falsiflow_troubleshooting.md" in readme_text and all(token in troubleshooting_text for token in ["claim_check_blocked", "external_blocked"]), "README links troubleshooting while the troubleshooting doc carries blocked command recovery paths.", troubleshooting_path)
        add("readme_security_posture_entry", "falsiflow_security_posture.md" in readme_text and all(token in maintainer_docs_text for token in ["Dependabot", "OpenSSF Scorecard", "release-check"]), "README links security posture while docs carry dependency automation, Scorecard, and release-gate guidance.", security_posture_path)
        add("readme_adoption_entry", all(token in docs_reference_text for token in ["falsiflow_adoption_priorities.md", "quickstart", "doctor", "adoption-check"]), "Docs surface adoption priorities and first-run path while README stays launch-focused.", adoption_priorities_path)
        add("readme_cli_reference_entry", "falsiflow_cli_reference.md" in readme_text and "falsiflow cli-reference" in cli_reference_text, "README links the generated CLI reference.", cli_reference_path)
        add("readme_positioning_entry", "falsiflow_positioning.md" in readme_text and all(token in positioning_text for token in ["ELN/LIMS", "Great Expectations", "Evidently", "Deepchecks", "MLflow"]), "README links positioning while the positioning doc carries neighboring-tool boundaries.", positioning_path)
        add("readme_public_casebook_entry", "falsiflow_public_casebook.md" in readme_text and all(token in public_casebook_text for token in ["Biointerface", "AI claim", "product metric", "vendor", "wetware"]), "README links the public casebook while the casebook doc names starter-template coverage.", public_casebook_path)
        add("readme_rag_quality_gate_proposal_entry", "falsiflow_rag_quality_gate_proposal.md" in readme_text and all(token in rag_quality_gate_proposal_text for token in ["RAG Quality Gate", "evaluation-set provenance", "retrieval quality", "answer faithfulness", "eval_set_pending"]), "README links the RAG quality gate proposal while the proposal carries evidence dimensions.", rag_quality_gate_proposal_path)
        add("readme_casebook_check_entry", "falsiflow_casebook_check.md" in readme_text and all(token in casebook_check_text for token in ["casebook-check", "positive demos", "placeholder blockers"]), "README links casebook checks while the casebook-check doc carries machine-verifiable proof paths.", casebook_check_path)
        add("readme_demo_pr_entry", "falsiflow_demo_pr_playbook.md" in readme_text and all(token in demo_pr_playbook_text for token in ["Blocked PR", "Ready PR", "placeholder", "source-backed evidence"]), "README links the public demo PR playbook while the playbook carries blocked and ready claim-gate guidance.", demo_pr_playbook_path)
        add("readme_downstream_pr_proof_strip_asset", all(token in readme_text + demo_pr_playbook_text + launch_plan_text for token in ["docs/assets/falsiflow_downstream_pr_proof_strip.svg", "falsiflow-downstream-ai-eval-demo", "26711652990", "26711669112", "falsiflow-downstream-rag-eval-demo", "26721829145", "26721856616", "falsiflow-downstream-product-metric-demo", "26726360229", "26726392921"]) and all(token in downstream_pr_proof_strip_text for token in ["AI Eval Downstream PR #1", "RAG Eval Downstream PR #1", "Product Metric Downstream PR #1", "claim_check_blocked", "claim_check_ready", "26711652990", "26711669112", "26721829145", "26721856616", "26726360229", "26726392921", "does not prove the model"]), "README and launch docs embed or link the shareable downstream PR proof strip with AI/RAG/product metric blocked and ready CI proof links.", downstream_pr_proof_strip_path)
        add("readme_benchmark_proof_links", all(token in readme_text + benchmark_article_text + launch_plan_text + github_action_examples_text for token in ["Benchmarks Should Fail Builds", "falsiflow-downstream-ai-eval-demo/pull/1", "26711652990", "26711669112", "evidence-package readiness", "model quality"]), "Launch docs link the benchmark-shaped proof article, live AI eval blocked/ready runs, and benchmark evidence-package boundary.", benchmark_article_path)
        add("readme_local_llm_proof_links", all(token in readme_text + local_llm_eval_text + local_llm_fixture_readme_text for token in ["docs/falsiflow_local_llm_eval.md", "examples/local_llm_eval_import", "coverage_ready", "claim_check_ready", "Ollama", "LM Studio", "llama.cpp", "does not run"]), "README links the local LLM fixture proof path while docs carry expected ready statuses and no-model/no-network boundaries.", local_llm_eval_path)
        add("readme_rag_import_proof_links", all(token in readme_text for token in ["RAG import proof path", "docs/falsiflow_rag_quality_gate_proposal.md", "examples/downstream_rag_eval_smoke", "coverage_ready", "claim_check_ready", "RAG evidence-package", "readiness", "--profile rag-eval"]), "README links the RAG import proof path with expected ready statuses and evidence-package readiness boundary.", readme_path)
        ...
        add("public_demo_downstream_rag_proof", all(token in public_demo_index_text + public_demo_downstream_pr_proof_strip_text for token in ["Live Downstream PR Story", "AI eval PR", "RAG eval PR", "Product metric PR", "falsiflow-downstream-ai-eval-demo", "falsiflow-downstream-rag-eval-demo", "falsiflow-downstream-product-metric-demo", "26711652990", "26711669112", "26721829145", "26721856616", "26726360229", "26726392921", "claim_check_blocked", "claim_check_ready"]), "Prebuilt public demo launchpad and proof-strip asset show the live AI, RAG, and product-metric downstream blocked-to-ready PR stories.", public_demo_index_path)
        add("readme_live_pr_story_reel_asset", readme_pr_story_reel_path.exists() and all(token in readme_pr_story_reel_text for token in ["Live PR Story", "PR #17", "claim_check_blocked", "claim_check_ready", "26708459093", "26708472653"]), "Packaged live PR story reel carries blocked and ready CI proof links without needing to crowd the README first screen.", readme_pr_story_reel_path)
        ...
        add("readme_visual_asset", any("falsiflow_downstream_pr_proof_strip.png" in target for target in readme_image_targets) and all(token in downstream_pr_proof_strip_text for token in ["AI Eval Downstream PR #1", "RAG Eval Downstream PR #1", "Product Metric Downstream PR #1", "claim_check_blocked", "claim_check_ready"]), "README embeds the first-screen downstream proof-strip PNG while the SVG asset carries the blocked-to-ready PR story.", downstream_pr_proof_strip_path)
        add("readme_30_second_demo_asset", all(token in readme_text for token in ["30 Seconds", "quickstart_ready", "claim_check_ready", "claim_check_blocked"]) and all(token in readme_demo_strip_text for token in ["30-second AI claim demo", "quickstart_ready", "claim_check_ready", "claim_check_blocked", "evidence_placeholder_demo.csv", "next actions"]), "README carries the 30-second ready-vs-blocked text while the packaged demo visual remains available for launch materials.", readme_demo_strip_path)
        ...
        add("readme_first_screen_story", all(token in readme_first_screen for token in ["AI eval", "product metric", "R&D", "falsiflow quickstart --template ai_claim_evaluation", "quickstart_ready", "claim_check_ready", "claim_check_blocked", "Public demo", "PyPI trusted publishing"]) and "GitHub Action" in readme_text, "README first screen names broad use cases, public demo, AI-claim quickstart, blocked placeholder behavior, and current PyPI trusted-publishing status, with Action adoption immediately below.", readme_path)
        add("start_docs", all(token in cli_docs_text + install_script_text + makefile_text for token in ["falsiflow start", "falsiflow start --check --json", "start-check:"]), "CLI docs and installer shortcuts document the beginner local app command.", cli_reference_path)
        add("install_docs", all(token in install_script_text + makefile_text for token in ["scripts/install_local.sh", "install-local:", "start:", "FALSIFLOW_REPO_URL", "python3 -m venv"]), "Installer docs and Makefile document one-command local installation.", install_script_path)
        ...
        add("pipx_docs", all(token in readme_text + makefile_text for token in ["pipx-install:", "pipx-start:", "pipx install --force ."]), "README and Makefile document pipx installation.", readme_path)
        add("onboard_docs", all(token in cli_docs_text + makefile_text for token in ["falsiflow onboard", "onboard-check:"]), "CLI docs document beginner onboarding.", cli_reference_path)
        add("static_demo_docs", all(token in launch_docs_text + cli_reference_text + makefile_text for token in ["falsiflow static-demo", "static-demo:", "GitHub Pages"]), "CLI and launch docs document static zero-install demo export.", cli_reference_path)
        add("demo_package_docs", all(token in launch_docs_text + cli_reference_text for token in ["falsiflow demo-package", "demo_package_summary.json", "FALSIFLOW_PUBLIC_DEMO_URL"]), "CLI and launch docs document public demo packaging and hosted URL handoff.", cli_reference_path)
        add("publish_kit_docs", all(token in launch_docs_text + cli_reference_text for token in ["falsiflow publish-kit", "publish_handoff.json", "public_release_evidence.json", "release_rehearsal.json", "Scorecard"]), "CLI and release docs document the public release handoff kit, evidence ledger, and account-bound rehearsal.", release_path)
        ...
        add(
            ...
            all(
                ...
                for token in [
                    ...
                    "claim_check_ready",
                    "Completed Work",
                    "Closed GitHub issues are the archive",
                    "falsiflow-downstream-ai-eval-demo",
                    "falsiflow-downstream-rag-eval-demo",
                    "falsiflow-downstream-product-metric-demo",
                    "falsiflow release-proof",
                    "release-check",
                ]
            ),
            "Public issue queue doc keeps GitHub issues as the source of truth, lists only current public entry points, and links durable proof anchors without mirroring closed issue state.",
            public_issue_queue_path,
        )
        ...
        add(
            ...
            all(
                token in product_metric_article_text
                for token in [
                    "## Live Proof",
                    "falsiflow-downstream-product-metric-demo PR #1",
                    "falsiflow-downstream-product-metric-demo/pull/1",
                    "analysis_pending",
                    "26726360229",
                    "claim_check_blocked",
                    "26726392921",
                    "claim_check_ready",
                    "source-backed metric provenance, lift",
                    "guardrail",
                    "rollback-readiness",
                    "reviewable evidence package",
                    "does not prove product impact, launch safety",
                ]
            ),
            "Product metric launch article links the live downstream product-metric PR, blocked and ready runs, and the evidence-package rather than product-impact boundary.",
            product_metric_article_path,
        )
        add(
            "benchmark_article_live_proof_docs",
            all(
                token in benchmark_article_text
                for token in [
                    "## Live Proof",
                    "falsiflow-downstream-ai-eval-demo PR #1",
                    "falsiflow-downstream-ai-eval-demo/pull/1",
                    "dataset_pending",
                    "26711652990",
                    "claim_check_blocked",
                    "source-backed benchmark-quality rows",
                    "raw eval export",
                    "26711669112",
                    "claim_check_ready",
                    "reviewable evidence package",
                    "does not decide benchmark correctness, model",
                    "trusted eval harness",
                    "incomplete benchmark",
                ]
            ),
            "Benchmark launch article links the live downstream AI-eval PR, blocked and ready runs, and the evidence-package rather than benchmark-correctness/model-quality boundary.",
            benchmark_article_path,
        )
        launch_execution_has_current_copy = all(
            ...
            for token in [
                ...
                "do not mirror every closed seed issue",
                "open PR #17",
                "issue #22",
                "issue #39",
                "short launch GIF",
                "good first issue",
                "live AI/RAG/product-metric downstream PR links",
                "falsiflow-downstream-ai-eval-demo",
                "26711652990",
                "26711669112",
                "falsiflow-downstream-rag-eval-demo",
        ...
        add("launch_execution_docs", launch_execution_has_current_copy and not launch_execution_has_stale_versioned_copy, "Launch execution doc records the current pre-public-post baseline, social-preview handoff, active queue, source-of-truth issue boundary, external evidence workflow, AI/RAG/product-metric downstream proof, and non-stale blocked-to-ready launch copy.", launch_execution_path)
        add("external_check_docs", all(token in launch_docs_text + github_external_evidence_text + pypi_trusted_publishing_text + release_text for token in ["falsiflow external-evidence", "falsiflow external-check", "falsiflow release-proof", "external_readiness.json", "external_ready", "external_blocked", "release_proof.md", "FALSIFLOW_PYPI_PACKAGE_URL", "https://pypi.org/pypi/falsiflow/json", "expected_version", "published_version", "Falsiflow External Evidence", "quickstart_ready", "doctor_ready", "claim_check_ready", "bundle_verified", "pypi_version_match=passed", "public_package_claim_check=passed", "falsiflow_external_evidence.json"]), "Release, launch, and PyPI docs surface structured external evidence, release-proof snippets, public package evidence, PyPI expected-version verification, and readiness checks.", pypi_trusted_publishing_path)
        action_ref_ready = f"uses: AzurLiu/falsiflow@v{current_version}" in readme_text
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text + github_action_examples_text + action_text for token in ["mode: claim-check", "evidence-import", "install-command", "GITHUB_ACTION_PATH", "falsiflow_github_action_examples.md", "examples/downstream_ai_eval_smoke", "examples/downstream_product_metric_smoke", "examples/downstream_rag_eval_smoke", "examples/local_llm_eval_import", "falsiflow-downstream-ai-eval-demo", "falsiflow-downstream-rag-eval-demo", "falsiflow-downstream-product-metric-demo", "26721829145", "26721856616", "26726360229", "26726392921", "template-check", "external-check"]), "README keeps the versioned Action snippet while Action docs carry evidence-import, install override, live downstream proof, and copy-paste examples.", readme_path)
        versioned_action_ref = f"AzurLiu/falsiflow@v{current_version}"
        add("github_action_examples_docs", all(token in github_action_examples_text for token in ["Clean Downstream Smoke Repo", "examples/downstream_ai_eval_smoke", "examples/downstream_product_metric_smoke", "examples/downstream_rag_eval_smoke", "examples/local_llm_eval_import", "falsiflow-downstream-ai-eval-demo", "26711652990", "26711669112", "falsiflow-downstream-rag-eval-demo", "26721829145", "26721856616", "falsiflow-downstream-product-metric-demo", "26726360229", "26726392921", "falsiflow_ai_eval/project.json", "falsiflow_ai_eval/evidence.csv", "falsiflow_product_metric/project.json", "falsiflow_product_metric/evidence.csv", "falsiflow_rag_eval/project.json", "falsiflow_rag_eval/evidence.csv", "evidence_placeholder_demo.csv", "evidence_pass_demo.csv", "Product Metric Downstream Smoke", "product_metric_blocked", "product_metric_ready", "rag_eval_blocked", "rag_eval_ready", "Local LLM Eval Import And Claim Gate", "mode: evidence-import", "profile: local-llm-eval", "coverage_ready", "Local reproduction before pushing", "python3 -m venv .venv", "downstream_blocked", "downstream_ready", "AI Eval Claim Gate", "actions/checkout@v6", "AzurLiu/falsiflow@vX.Y.Z", "actions/upload-artifact@v7", "summary-json", "summary-md", "GITHUB_ACTION_PATH", "install-command", "claim_check_ready", "claim_check_blocked", "falsiflow_demo_pr_playbook.md"]), "GitHub Action examples document maintained AI eval, product metric, RAG eval, local LLM, and live RAG/product-metric fixtures/proof, exact local venv reproduction, claim gates, evidence-import mode, artifact uploads, default action checkout installs, install overrides, ready/blocked statuses, and the public demo PR playbook.", github_action_examples_path)
        add(
        ...
        add(
            "downstream_rag_eval_live_proof_links",
            all(
                token in readme_text + demo_pr_playbook_text + github_action_examples_text + launch_execution_text
                for token in [
                    "falsiflow-downstream-rag-eval-demo",
                    "26721829145",
                    "26721856616",
            ...
            "README, demo playbook, GitHub Action examples, and launch execution docs link the live downstream blocked-to-ready RAG eval PR proof.",
            github_action_examples_path,
        )
        add(
        ...
        add(
            ...
            all(token in downstream_rag_smoke_workflow_text for token in ["Falsiflow Downstream RAG Eval Smoke", "actions/checkout@v6", versioned_action_ref, "mode: claim-check", "project-dir: falsiflow_rag_eval", "evidence: falsiflow_rag_eval/evidence.csv", "actions/upload-artifact@v7"])
            and all(token in downstream_rag_smoke_readme_text for token in ["copy-paste downstream repository fixture", "RAG eval claim", "retrieval", "faithfulness", "citation", "claim_check_blocked", "claim_check_ready", "evidence_pass_demo.csv", "evidence_placeholder_demo.csv"])
            and "eval_set_pending" in downstream_rag_smoke_evidence_text
            and downstream_rag_smoke_evidence_text == downstream_rag_smoke_placeholder_text
            and all(token in downstream_rag_smoke_pass_text for token in ["recall_at_5,0.86", "faithfulness_rate,0.94", "citation_precision,0.93", "raw_answers_archived,true"]),
            "Downstream RAG eval smoke fixture is a copy-paste repo skeleton whose default evidence is blocked and whose pass evidence is source-backed.",
            downstream_rag_smoke_path,
        )
        add(
            "local_llm_eval_import_fixture",
            all(token in local_llm_fixture_workflow_text for token in ["Falsiflow Local LLM Eval Import", "actions/checkout@v6", versioned_action_ref, "mode: evidence-import", "profile: local-llm-eval", "source-file: source_files/local_eval_results.jsonl", "mode: claim-check", "actions/upload-artifact@v7"])
            and all(token in local_llm_fixture_readme_text for token in ["copy-paste fixture", "Ollama", "LM Studio", "llama.cpp", "claim_check_blocked", "claim_check_ready", "coverage_ready", "local_model_manifest.json", "does not run the model", "open an API port"])
            and "dataset_pending" in local_llm_fixture_evidence_text
            and local_llm_fixture_evidence_text == local_llm_fixture_placeholder_text
            and all(token in local_llm_fixture_imported_text for token in ["exact_match_rate,0.86", "raw_outputs_archived,true", "regression_ci_run_recorded,true"])
            and all(token in local_llm_fixture_manifest_text for token in ["llama.cpp", "decode_params", "model_file_hash", "local://llama-cpp-eval-run-001"])
            and all(token in local_llm_fixture_source_text for token in ["candidate_model", "baseline_model", "evaluated_item_count"]),
            "Local LLM eval import fixture is a copy-paste artifact-first path whose default evidence blocks and whose imported local-runner artifacts are source-backed.",
            local_llm_fixture_path,
        )
        add("workbench_docs", all(token in docs_reference_text for token in ["workbench.html", "ready/blocked status", "review flow", "evidence lineage", "repair checklist", "source manifest", "bundle verification"]), "Docs document the browser workbench closed loop without crowding the README launch surface.", adoption_priorities_path)
        add("discover_docs", all(token in public_interface_docs_text + docs_reference_text for token in ["falsiflow discover", "evidence_records.json", "candidate_queue.json", "claim_ready=false"]), "CLI and architecture docs document structured discovery outputs and the non-claim boundary.", cli_reference_path)
        add("public_interface_docs", all(token in public_interface_docs_text for token in ["falsiflow agent discover", "falsiflow candidate rank", "falsiflow assay-plan", "falsiflow evidence import"]), "CLI docs document the structured discovery and evidence public interfaces.", cli_reference_path)
        ...
        add("github_action_evidence_import_mode", all(token in action_text for token in ["evidence-import", "FALSIFLOW_PROFILE", "FALSIFLOW_INPUT", "FALSIFLOW_MANIFEST", "FALSIFLOW_SOURCE_FILE", "summary-out", "coverage-out", "evidence import --profile", "--summary-out", "--coverage-out"]), "Reusable GitHub Action supports evidence-import mode for converting local/private eval artifacts before claim-check.", action_path)
        ...
        add("github_external_evidence_workflow", all(token in github_external_evidence_text for token in ["Falsiflow External Evidence", "public_demo_url", "pypi_package_url", "expected_version", "Expected PyPI package version", "Verify PyPI project URL", "https://pypi.org/pypi/falsiflow/json", "falsiflow_pypi_project.json", "falsiflow_expected_version.txt", "falsiflow_pypi_version.txt", "published_version", "tomllib", "pipx-smoke", "pipx-public-package", "windows-powershell", "pypi_package_url", "pipx_public_package", "public_package_first_run", "public_package_claim_check", "falsiflow_public_package_first_run_quickstart.json", "falsiflow_public_package_first_run_doctor.json", "falsiflow_public_package_claim_check.json", "mcp_public_package_selftest", "falsiflow_public_package_mcp_selftest.json", "falsiflow quickstart --template ai_claim_evaluation", "falsiflow doctor --project-dir", "falsiflow claim-check --project-dir", "falsiflow mcp --selftest --json", "falsiflow_external_evidence.json", "external-check", "upload-artifact"]), "GitHub Actions external-evidence workflow captures hosted demo, PyPI JSON expected-version package proof, checkout pipx, public-package pipx, public-package first-run quickstart/doctor, public-package claim-check, public-package MCP selftest, Windows, and external-check artifacts for public readiness proof.", github_external_evidence_path)
        ...
        add("try_docs", all(token in cli_docs_text + public_demo_index_text for token in ["falsiflow try", "try_report.html"]), "CLI and public demo docs document the low-friction local browser demo.", cli_reference_path)
        add("launchpad_docs", public_demo_index_path.exists() and all(token in launch_docs_text + public_demo_index_text for token in ["index.html", "try_report_url", "wizard_url"]), "Launch docs and public demo artifacts document the first-run local browser launchpad.", public_demo_index_path)
        add("wizard_docs", all(token in cli_docs_text for token in ["falsiflow wizard", "falsiflow_wizard.html"]), "CLI docs document the browser wizard for drafting claim gates.", cli_reference_path)
        add("serve_docs", all(token in cli_docs_text for token in ["falsiflow serve", "localhost", "serve_summary.json", "try_report.html"]), "CLI docs document the localhost browser demo.", cli_reference_path)
        add("quickstart_docs", all(token in readme_text + cli_docs_text + release_text + data_contract_text for token in ["quickstart", "quickstart_summary.json", "next_commands", "quickstart_ready", "quickstart_blocked"]), "README and CLI docs document the one-command first-run quickstart.", cli_reference_path)
        add("doctor_docs", all(token in readme_text + cli_docs_text + release_text for token in ["doctor", "doctor_summary.json", "repair_checklist", "Falsiflow Doctor", "doctor_ready", "doctor_blocked"]), "README and CLI docs document the one-command project health diagnosis.", cli_reference_path)
        add("claim_check_docs", all(token in readme_text + cli_docs_text + data_contract_text + public_casebook_text for token in ["claim-check", "--project-dir", "claim_check.json", "claim_check_ready", "claim_check_blocked", "source manifest", "bundle verification"]), "README and CLI docs document the one-command claim gate and review artifacts.", cli_reference_path)
        add("adoption_check_docs", all(token in cli_docs_text + adoption_priorities_text + release_text for token in ["adoption-check", "adoption_check.json", "adoption_ready", "adoption_blocked", "repair_checklist", "adoption_recheck"]), "CLI and adoption docs document the priority-readiness adoption gate.", adoption_priorities_path)
        add("audit_review_docs", all(token in docs_reference_text for token in ["audit_review.json", "audit_review.md", "review_ready"]), "Docs document trusted audit review decision cards.", data_contract_path)
        add("template_check_docs", all(token in template_docs_text + data_contract_text for token in ["template-check", "placeholder", "template_ready"]), "Template docs document external template authoring checks.", template_authoring_path)
        add("template_scaffold_docs", all(token in template_docs_text for token in ["template-scaffold", "evidence_pass_demo.csv", "template_scaffolded"]), "Template docs document starter template generation.", template_authoring_path)
        add("template_pack_docs", all(token in template_docs_text for token in ["template-pack", "verify-template-pack", "template_pack_verified"]), "Template docs document starter template packaging and verification.", template_authoring_path)
        add("template_install_docs", all(token in template_docs_text for token in ["template-install", "template_installed"]), "Template docs document verified starter template installation.", template_authoring_path)
        add("template_registry_docs", all(token in template_docs_text for token in ["template-registry", "template-lock", "falsiflow_template_lock.json"]), "Template docs document registry and lockfile workflows.", template_authoring_path)
        add("template_attestation_docs", all(token in template_docs_text for token in ["template-attest", "verify-template-attestation", "template_attestation_verified"]), "Template docs document template lock attestations and verification.", template_authoring_path)
        add("template_policy_docs", all(token in template_docs_text for token in ["template-policy", "verify-template-policy", "template_policy_verified"]), "Template docs document template adoption policies.", template_authoring_path)
        add("template_release_docs", all(token in template_docs_text for token in ["template-release", "verify-template-release", "template_release_verified", "template_release_verification.md"]), "Template docs document template release bundles and release artifact indexes.", template_authoring_path)
        add("template_gallery_docs", all(token in docs_reference_text for token in ["template-gallery", "rfq_vendor_evidence", "biointerface_coatings", "wetware_support_hardware", "ai_claim_evaluation", "product_metric_launch"]), "Docs document the template gallery and cross-domain starter breadth.", public_casebook_path)
        ...
        add("demo_pr_playbook_docs", all(token in demo_pr_playbook_text for token in ["Falsiflow Demo PR Playbook", "Tiny LLM Eval Fixture", "dataset_pending", "exact_match_rate", "does not prove", "Blocked PR", "Ready PR", "evidence_placeholder_demo.csv", "evidence_pass_demo.csv", "claim_check_blocked", "claim_check_ready", "falsiflow-downstream-product-metric-demo", "26726360229", "26726392921", "AzurLiu/falsiflow@vX.Y.Z", "actions/upload-artifact@v7", "GITHUB_ACTION_PATH", "30-second Recording Script"]), "Demo PR playbook documents public blocked-to-ready AI eval and product-metric claim gate demonstrations, tiny LLM eval fixture, reusable action setup, recording guidance, and responsible-use boundary.", demo_pr_playbook_path)
        ...
        add("cli_reference_docs", all(token in cli_reference_text for token in ["Falsiflow CLI Reference", "falsiflow start", "falsiflow cli-reference", "falsiflow mcp", "--selftest", "mcp_selftest_ready", "falsiflow release-check", "falsiflow release-proof", "falsiflow external-evidence", "falsiflow evidence import", "falsiflow template-release"]), "Generated CLI reference documents first-run, MCP selftest, release, release-proof, external evidence, import, and template commands.", cli_reference_path)
    ...
    if sdist_path and sdist_path.exists():
        try:
            ...
            required = {
                ...
                f"{base}/examples/downstream_ai_eval_smoke/README.md",
                f"{base}/examples/downstream_ai_eval_smoke/.github/workflows/falsiflow-ai-eval.yml",
                f"{base}/examples/downstream_ai_eval_smoke/falsiflow_ai_eval/project.json",
                f"{base}/examples/downstream_ai_eval_smoke/falsiflow_ai_eval/evidence.csv",
                f"{base}/examples/downstream_ai_eval_smoke/falsiflow_ai_eval/evidence_pass_demo.csv",
                f"{base}/examples/downstream_ai_eval_smoke/falsiflow_ai_eval/evidence_placeholder_demo.csv",
                f"{base}/examples/downstream_ai_eval_smoke/falsiflow_ai_eval/source_files/ai_eval_raw_export.csv",
                ...
                f"{base}/examples/downstream_rag_eval_smoke/README.md",
                f"{base}/examples/downstream_rag_eval_smoke/.github/workflows/falsiflow-rag-eval.yml",
                f"{base}/examples/downstream_rag_eval_smoke/falsiflow_rag_eval/project.json",
                f"{base}/examples/downstream_rag_eval_smoke/falsiflow_rag_eval/evidence.csv",
                f"{base}/examples/downstream_rag_eval_smoke/falsiflow_rag_eval/evidence_pass_demo.csv",
                f"{base}/examples/downstream_rag_eval_smoke/falsiflow_rag_eval/evidence_placeholder_demo.csv",
                f"{base}/examples/downstream_rag_eval_smoke/falsiflow_rag_eval/source_files/rag_eval_raw_export.csv",
                f"{base}/examples/local_llm_eval_import/README.md",
                f"{base}/examples/local_llm_eval_import/.github/workflows/falsiflow-local-llm-eval.yml",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/project.json",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/evidence.csv",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/evidence_imported_demo.csv",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/evidence_placeholder_demo.csv",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/local_model_manifest.json",
                f"{base}/examples/local_llm_eval_import/falsiflow_local_llm_eval/source_files/local_eval_results.jsonl",
    ...
    try:
        ...
        if demo_package_summary.get("status") != "demo_package_ready":
            ...
            if not all(token in demo_index_text + demo_strip_text for token in demo_tokens):
                failures.append(failure_record("demo_package", "live_downstream_proof", "Generated public demo package is missing AI/RAG/product-metric downstream blocked-to-ready proof links."))
    except Exception as exc:  # pragma: no cover - defensive CLI boundary.
