...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        action_ref_ready = f"uses: AzurLiu/falsiflow@v{current_version}" in readme_text or "uses: AzurLiu/falsiflow@main" in readme_text
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text for token in ["action.yml", "mode: claim-check", "install-command", "GITHUB_ACTION_PATH", "template-check", "external-check"]), "README documents downstream reusable GitHub Action adoption, default action-checkout installation, and install override.", readme_path)
        ...
        add("github_action_entrypoint", all(token in action_text for token in ["using: composite", "actions/setup-python@v5", "install-command", "GITHUB_ACTION_PATH", "mode", "claim-check", "template-check", "casebook-check", "release-check", "adoption-check", "quickstart", "external-check", "summary-json", "GITHUB_OUTPUT"]), "Reusable GitHub Action exposes supported Falsiflow gates, Python setup, default action-checkout install, install override, and output metadata.", action_path)
