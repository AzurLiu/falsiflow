...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text for token in ["action.yml", "mode: claim-check", "install-command", "GITHUB_ACTION_PATH", "falsiflow_github_action_examples.md", "six-file downstream smoke repo", "template-check", "external-check"]), "README documents downstream reusable GitHub Action adoption, versioned action pinning, default action-checkout installation, install override, and copy-paste examples.", readme_path)
