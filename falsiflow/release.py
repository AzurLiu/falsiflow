...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text for token in ["action.yml", "mode: claim-check", "install-command", "GITHUB_ACTION_PATH", "falsiflow_github_action_examples.md", "template-check", "external-check"]), "README documents downstream reusable GitHub Action adoption, default action-checkout installation, install override, and copy-paste examples.", readme_path)
        add("github_action_examples_docs", all(token in github_action_examples_text for token in ["AI Eval Claim Gate", "actions/checkout@v6", "AzurLiu/falsiflow@main", "actions/upload-artifact@v7", "summary-json", "summary-md", "GITHUB_ACTION_PATH", "install-command", "claim_check_ready", "claim_check_blocked"]), "GitHub Action examples document AI eval claim gates, artifact uploads, default action checkout installs, install overrides, and ready/blocked statuses.", github_action_examples_path)
