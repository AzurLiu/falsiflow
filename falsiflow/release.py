...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("readme_mcp_entry", all(token in readme_text for token in ["falsiflow_mcp.md", "falsiflow mcp --selftest --json", "mcp_selftest_ready", "falsiflow mcp", "mcpServers", "\"command\": \"falsiflow\"", "\"args\": [\"mcp\"]", "no HTTP listener", "does not run a model"]), "README documents the local stdio MCP server, one-command selftest, copy-paste client shape, and no-network/no-model boundary for agent integrations.", readme_path)
        ...
        add("readme_benchmark_proof_links", all(token in readme_text for token in ["Benchmarks Should Fail Builds", "docs/launch_articles/benchmarks_should_fail_builds.md", "falsiflow-downstream-ai-eval-demo/pull/1", "26711652990", "26711669112", "benchmark-shaped proof", "evidence-package readiness", "not benchmark correctness or", "model quality"]), "README links the benchmark-shaped proof article, live AI eval blocked/ready runs, and benchmark evidence-package boundary.", readme_path)
        add("readme_local_llm_proof_links", all(token in readme_text for token in ["docs/falsiflow_local_llm_eval.md", "examples/local_llm_eval_import", "coverage_ready", "claim_check_ready", "no-model-execution", "no-network-listener", "local Ollama", "LM Studio", "llama.cpp", "private-runner artifacts"]), "README links the local LLM fixture proof path with expected ready statuses and no-model/no-network boundary.", readme_path)
        ...
        add(
            ...
            all(
                ...
                for token in [
                    ...
                    "docs/falsiflow_rag_quality_gate_proposal.md",
                    "examples/downstream_rag_eval_smoke",
                    "RAG evidence-package readiness boundary",
                    "issues/36",
                    "RAG eval import proof snippet",
                    "falsiflow evidence import --profile",
                    "rag-eval",
                    "strict `claim-check`",
                    "issues/35",
                    "README local model section",
                    "docs/falsiflow_local_llm_eval.md",
                    "examples/local_llm_eval_import",
                    "coverage_ready",
                    "no-network-listener",
                    "issues/34",
                    "local LLM eval import proof snippet",
                    "bundle_verified",
                    "no-model-execution",
                    ...
                    "issues/32",
                    "benchmarks launch article",
                    "docs/launch_articles/benchmarks_should_fail_builds.md",
                    "26711652990",
                    "26711669112",
                    "benchmark correctness/model-quality",
                    "issues/31",
                    "product metrics launch article",
                    "docs/launch_articles/evidence_gates_for_product_metrics.md",
                    "issues/29",
                    "launch-article visual",
                    "docs/assets/falsiflow_downstream_pr_proof_strip.svg",
                    "docs/launch_articles/stop_shipping_unverifiable_ai_eval_claims.md",
                    "issues/30",
                    ...
                    "26726392921",
                    "issues/26",
                    "Generate a release proof snippet",
                    "issues/27",
                    "live downstream RAG eval claim-gate demo",
            ...
            "Public issue queue doc links current launch metrics and good-first issues plus completed seed issues with labels, goals, verification commands, and live RAG/product-metric/benchmark/local-LLM proof links.",
        ...
        add("launch_execution_docs", launch_execution_has_current_copy and not launch_execution_has_stale_versioned_copy, "Launch execution doc records the current pre-public-post baseline, social-preview handoff, active queue, completed seed issues, external evidence workflow, AI/RAG/product-metric downstream proof, and non-stale blocked-to-ready launch copy.", launch_execution_path)
        ...
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text for token in ["action.yml", "mode: claim-check", "mode: evidence-import", "install-command", "GITHUB_ACTION_PATH", "falsiflow_github_action_examples.md", "examples/downstream_ai_eval_smoke", "examples/downstream_product_metric_smoke", "examples/downstream_rag_eval_smoke", "examples/local_llm_eval_import", "falsiflow-downstream-ai-eval-demo", "falsiflow-downstream-rag-eval-demo", "falsiflow-downstream-product-metric-demo", "26721829145", "26721856616", "26726360229", "26726392921", "template-check", "external-check"]), "README documents downstream reusable GitHub Action adoption, versioned action pinning, evidence-import mode, default action-checkout installation, install override, live AI/RAG/product-metric downstream proof, and copy-paste examples.", readme_path)
