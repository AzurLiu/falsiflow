...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("github_action_examples_docs", all(token in github_action_examples_text for token in ["Clean Downstream Smoke Repo", "falsiflow_ai_eval/project.json", "falsiflow_ai_eval/evidence.csv", "evidence_placeholder_demo.csv", "evidence_pass_demo.csv", "Local reproduction before pushing", "python3 -m venv .venv", "downstream_blocked", "downstream_ready", "AI Eval Claim Gate", "actions/checkout@v6", "AzurLiu/falsiflow@main", "actions/upload-artifact@v7", "summary-json", "summary-md", "GITHUB_ACTION_PATH", "install-command", "claim_check_ready", "claim_check_blocked", "falsiflow_demo_pr_playbook.md"]), "GitHub Action examples document a clean downstream smoke repo, exact local venv reproduction, AI eval claim gates, artifact uploads, default action checkout installs, install overrides, ready/blocked statuses, and the public demo PR playbook.", github_action_examples_path)
