...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("public_issue_queue_docs", all(token in public_issue_queue_text for token in ["Active Public Issues", "issues/26", "Generate a release proof snippet", "issues/27", "live downstream RAG eval claim-gate demo", "issues/28", "MCP client configuration examples", "good first issue", "help wanted", "release-check", "mcp_selftest_ready"]), "Public issue queue doc links current contributor-facing issues with labels, goals, and verification commands.", public_issue_queue_path)
