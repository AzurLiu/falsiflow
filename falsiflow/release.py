...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("demo_pr_playbook_docs", all(token in demo_pr_playbook_text for token in ["Falsiflow Demo PR Playbook", "Blocked PR", "Ready PR", "evidence_placeholder_demo.csv", "evidence_pass_demo.csv", "claim_check_blocked", "claim_check_ready", "AzurLiu/falsiflow@main", "actions/upload-artifact@v7", "GITHUB_ACTION_PATH", "30-second Recording Script"]), "Demo PR playbook documents a public blocked-to-ready AI eval claim gate demonstration with reusable action setup and recording guidance.", demo_pr_playbook_path)
        ...
        add("positioning_casebook", all(token in positioning_text for token in ["Comparison Map", "Named Tool Boundaries", "Casebook", "Spreadsheet checklist", "CI test suite", "ELN or LIMS", "ML eval harness", "Materials database", "Workflow orchestrator", "Great Expectations", "Evidently", "Deepchecks", "MLflow", "Plain GitHub Actions", "Decision Rubric"]), "Positioning doc explains when Falsiflow fits and how it differs from adjacent tool categories and named neighboring tools.", positioning_path)
