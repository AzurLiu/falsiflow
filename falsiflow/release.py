...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("public_issue_queue_docs", all(token in public_issue_queue_text for token in ["Active Public Issues", "issues/22", "launch metrics review", "issues/26", "Generate a release proof snippet", "issues/27", "live downstream RAG eval claim-gate demo", "issues/28", "MCP client configuration examples", "Completed Seed Issues", "falsiflow release-proof", "good first issue", "help wanted", "release-check", "mcp_selftest_ready"]), "Public issue queue doc links current contributor-facing issues plus completed seed issues with labels, goals, and verification commands.", public_issue_queue_path)
        add("launch_execution_docs", all(token in launch_execution_text for token in ["Falsiflow Launch Execution", "Pre-public-post baseline", "0 stars", "2 forks", "open issues #22 and #27", "completed seed issues #26 and", "open PR #17", "falsiflow-external-evidence.yml", "release proof snippet generation", "MCP client configuration examples", "24-hour post-public-post metrics review", "claim_check_blocked", "claim_check_ready"]), "Launch execution doc records the current pre-public-post baseline, active queue, completed seed issues, external evidence workflow, and blocked-to-ready launch copy.", launch_execution_path)
        ...
        add("readme_github_action_docs", action_ref_ready and all(token in readme_text for token in ["action.yml", "mode: claim-check", "mode: evidence-import", "install-command", "GITHUB_ACTION_PATH", "falsiflow_github_action_examples.md", "examples/downstream_ai_eval_smoke", "examples/downstream_product_metric_smoke", "examples/local_llm_eval_import", "falsiflow-downstream-ai-eval-demo", "template-check", "external-check"]), "README documents downstream reusable GitHub Action adoption, versioned action pinning, evidence-import mode, default action-checkout installation, install override, live downstream proof, and copy-paste examples.", readme_path)
        versioned_action_ref = f"AzurLiu/falsiflow@v{current_version}"
        add("github_action_examples_docs", all(token in github_action_examples_text for token in ["Clean Downstream Smoke Repo", "examples/downstream_ai_eval_smoke", "examples/downstream_product_metric_smoke", "examples/local_llm_eval_import", "falsiflow-downstream-ai-eval-demo", "26711652990", "26711669112", "falsiflow_ai_eval/project.json", "falsiflow_ai_eval/evidence.csv", "falsiflow_product_metric/project.json", "falsiflow_product_metric/evidence.csv", "evidence_placeholder_demo.csv", "evidence_pass_demo.csv", "Product Metric Downstream Smoke", "product_metric_blocked", "product_metric_ready", "Local LLM Eval Import And Claim Gate", "mode: evidence-import", "profile: local-llm-eval", "coverage_ready", "Local reproduction before pushing", "python3 -m venv .venv", "downstream_blocked", "downstream_ready", "AI Eval Claim Gate", "actions/checkout@v6", versioned_action_ref, "actions/upload-artifact@v7", "summary-json", "summary-md", "GITHUB_ACTION_PATH", "install-command", "claim_check_ready", "claim_check_blocked", "falsiflow_demo_pr_playbook.md"]), "GitHub Action examples document maintained AI eval, product metric, and local LLM eval fixtures, live downstream proof repo, exact local venv reproduction, claim gates, evidence-import mode, artifact uploads, default action checkout installs, install overrides, ready/blocked statuses, and the public demo PR playbook.", github_action_examples_path)
        add(
