...
def package_release_checks(root: Path) -> dict[str, object]:
    ...
    if pyproject_path.exists():
        ...
        add("github_scorecard_workflow", all(token in github_scorecard_text for token in ["Falsiflow Scorecard", "ossf/scorecard-action@v2.4.3", "results_format: sarif", "scorecard-results.sarif", "github/codeql-action/upload-sarif", "security-events: write", "id-token: write"]), "GitHub Actions Scorecard workflow records OpenSSF Scorecard SARIF security signals.", github_scorecard_path)
