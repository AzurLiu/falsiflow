...
BITMAP_FONT_5X7: dict[str, tuple[str, ...]] = {
...
def write_rgb_png(path: Path, width: int, height: int, pixels: bytearray) -> None:
    ...
    path.write_bytes(b"\x89PNG\r\n\x1a\n" + png_chunk(b"IHDR", ihdr) + png_chunk(b"IDAT", zlib.compress(bytes(raw), level=9)) + png_chunk(b"IEND", b""))
...
def draw_text(pixels: bytearray, width: int, height: int, x: int, y: int, text: str, color: tuple[int, int, int], scale: int = 4) -> None:
    cursor = x
    for char in text.upper():
        ...
        for row_index, row in enumerate(glyph):
            for col_index, bit in enumerate(row):
                if bit == "1":
                    draw_rect(pixels, width, height, cursor + col_index * scale, y + row_index * scale, scale, scale, color)
        cursor += 6 * scale


def write_social_preview_png(path: Path, summary: dict[str, object]) -> None:
    width = 1280
    height = 640
    background = rgb("#f8fbff")
    pixels = bytearray(background * (width * height))
    draw_rect(pixels, width, height, 64, 64, 1152, 512, rgb("#ffffff"))
    draw_rect(pixels, width, height, 64, 64, 1152, 6, rgb("#d7e3ef"))
    draw_rect(pixels, width, height, 64, 570, 1152, 6, rgb("#d7e3ef"))
    draw_rect(pixels, width, height, 64, 64, 6, 512, rgb("#d7e3ef"))
    draw_rect(pixels, width, height, 1210, 64, 6, 512, rgb("#d7e3ef"))
    draw_text(pixels, width, height, 112, 128, "FALSIFLOW", rgb("#122033"), scale=11)
    draw_text(pixels, width, height, 116, 230, "STOP UNVERIFIABLE CLAIMS", rgb("#253449"), scale=5)
    draw_text(pixels, width, height, 116, 282, "FROM PASSING CI", rgb("#253449"), scale=5)
    draw_rect(pixels, width, height, 116, 390, 320, 72, rgb("#fff1f0"))
    draw_text(pixels, width, height, 146, 414, "BLOCKED BY CI", rgb("#9b2d24"), scale=3)
    draw_rect(pixels, width, height, 480, 390, 54, 72, rgb("#eef6ff"))
    draw_text(pixels, width, height, 494, 414, ">", rgb("#2c5b88"), scale=4)
    draw_rect(pixels, width, height, 578, 390, 282, 72, rgb("#eaf8ef"))
    draw_text(pixels, width, height, 608, 414, "READY TO MERGE", rgb("#205e35"), scale=3)
    external_state = str(summary.get("external_status", "external_blocked"))
    external_label = "EXT READY" if external_state == "external_ready" else "EXT CHECK"
    draw_rect(pixels, width, height, 914, 148, 226, 278, rgb("#f8fafc"))
    draw_text(pixels, width, height, 942, 188, "LIVE PROOF", rgb("#475569"), scale=3)
    draw_text(pixels, width, height, 942, 252, "AI EVAL", rgb("#122033"), scale=3)
    draw_text(pixels, width, height, 942, 316, "RAG EVAL", rgb("#122033"), scale=3)
    draw_text(pixels, width, height, 942, 382, external_label, rgb("#52677f"), scale=3)
    draw_text(pixels, width, height, 116, 510, "LOCAL FIRST  NO CLOUD UPLOAD  JSON + MARKDOWN + ZIP BUNDLE", rgb("#52677f"), scale=3)
...
def render_github_repo_profile(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        "## Social Preview",
        "",
        "Upload `social_preview.png` in GitHub repository settings after reviewing the generated image. Keep `social_preview.svg` as the editable source.",
        "",
        "## Pinned Links",
        "",
...
def render_launch_readme(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        f"- Social preview PNG: `{outputs.get('social_preview_png', '')}`",
        f"- Social preview SVG source: `{outputs.get('social_preview', '')}`",
...
def run_launch_kit(
    ...
    *,
    ...
    outputs = {
        ...
        "social_preview_png": str(out_dir / "social_preview.png"),
    ...
    write_social_preview_png(out_dir / "social_preview.png", summary)
