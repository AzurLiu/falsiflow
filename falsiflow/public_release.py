...
def render_launch_posts(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "Use these as drafts; replace placeholder URLs with the final public repository, hosted demo, and release evidence before posting.",
        "",
        "## Hacker News",
        "",
        "Title: Show HN: Falsiflow - evidence gates for risky R&D claims",
        "",
        "I built Falsiflow, a local-first CLI and static demo for turning high-risk technical claims into explicit evidence gates. A claim only becomes `claim_ready` when evidence rows, required source files, derived metrics, bundles, and release checks agree. Missing provenance or placeholder rows stay blocked.",
        "",
        f"Repo: {repo_url}",
        f"Demo: {public_demo_url}",
        "",
        "It is not an ELN/LIMS, not a regulatory system, and not scientific proof. It is a reproducible audit layer for deciding whether supplied evidence is complete enough to move forward.",
        "",
        "## Reddit / Community Post",
        "",
        "I am open-sourcing Falsiflow, a small evidence-gated workflow engine for R&D decisions. It packages claim checks, source-file provenance, audit reports, template release bundles, and local browser demos into one repeatable gate. I would love feedback from people who have watched spreadsheet-based evidence review drift over time.",
        "",
        f"- Repo: {repo_url}",
        f"- Demo: {public_demo_url}",
        ...
        "## LinkedIn",
        "",
        "Falsiflow is now ready for public review: an evidence-gated R&D workflow engine for high-risk technical claims. It blocks claims that rely on missing provenance, malformed evidence, placeholder rows, or ambiguous thresholds, then emits JSON/Markdown audit artifacts and portable bundles.",
        "",
        "The goal is not to replace lab systems or prove experimental truth. It is to make the handoff between claim, evidence, source files, and release checks explicit enough that reviewers can trust what moved forward and why.",
        "",
        f"External readiness at launch-kit time: `{external_status}`.",
        "",
        "## Short Post",
        "",
        f"Falsiflow turns risky R&D claims into auditable evidence gates: claim -> evidence rows -> source provenance -> review bundle -> ready/blocked. Try it: {public_demo_url} Repo: {repo_url}",
        "",
    ])
...
def render_launch_announcement(summary: dict[str, object]) -> str:
    return "\n".join([
        ...
        "",
        "Falsiflow is an evidence-gated R&D workflow engine for high-risk technical claims.",
        "",
        "It turns a claim into explicit gates, required evidence rows, source-file policy, derived metrics, and acceptance rules. A claim only becomes ready when configuration, evidence, provenance, bundles, and verification artifacts agree.",
        "",
        "## Try It",
        "",
        "```bash",
...
def render_launch_demo_script(summary: dict[str, object]) -> str:
    return "\n".join([
        ...
        "## 0:00 Problem",
        "",
        "High-risk research and vendor decisions often advance with missing provenance, placeholder evidence, or ambiguous thresholds. Falsiflow blocks that drift.",
        "",
        "## 0:30 Local Browser",
        "",
        "```bash",
        ...
        "```",
        "",
        f"End by noting that external readiness is `{summary.get('external_status', '')}` until public hosting, pipx, and Windows validation evidence are recorded.",
        "",
    ])
...
def launch_metrics_plan(summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "funnel": [
            ...
            {
                "stage": "verification",
                "signal": "release-check, casebook replay, or pipx smoke run",
                ...
                "action": "Link directly to casebook replay and release-check evidence from launch replies.",
            },
            {
                "stage": "conversion",
        ...
        "review_windows": [
            {
                ...
                "checks": [
                    ...
                    "launch posts include the responsible-use boundary",
                ],
            },
            {
                "window": "day-0",
