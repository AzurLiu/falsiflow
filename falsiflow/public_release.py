...
def render_launch_posts(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "Use these as drafts; replace placeholder URLs with the final public repository, hosted demo, demo PR, and release evidence before posting.",
        "",
        "## Channel Checklist",
        "",
        ...
        "| --- | --- | --- | --- |",
        "| Hacker News / Show HN | Concise technical launch with a working demo. | Confirm PyPI status, demo URL, release evidence, and responsible-use boundary. | Watch the first 60 minutes for install failures and unclear positioning questions. |",
        "| Reddit / community threads | Feedback from people running AI eval, product analytics, R&D review, or CI gates. | Choose one relevant community at a time and include the demo PR playbook. | Turn repeated questions into docs or labeled GitHub issues within 24 hours. |",
        ...
        "## Hacker News",
        "",
        "Title: Show HN: Falsiflow - CI gates for unsupported AI eval claims",
        "",
        ...
        f"Demo PR playbook: {demo_pr_playbook_url}",
        "",
        "The fastest demo is a PR that first fails because `evidence_placeholder_demo.csv` backs an AI-eval claim, then turns green after the PR swaps in `evidence_pass_demo.csv`. It is not an ELN/LIMS, not a regulatory system, and not scientific proof. It is a reproducible audit layer for deciding whether supplied evidence is complete enough to move forward.",
        "",
        ...
        "- CI gate: `uses: AzurLiu/falsiflow@main` with `mode: claim-check`",
        ...
        "",
        "The demo PR path shows the core behavior: placeholder evidence produces `claim_check_blocked`; source-backed evidence produces `claim_check_ready`; reviewers get JSON, Markdown, source manifests, dashboards, and a bundle verification report.",
        "",
        f"Demo PR playbook: {demo_pr_playbook_url}",
        "",
        ...
        "",
        "2/ The quickest demo is a PR: placeholder AI-eval evidence fails as `claim_check_blocked`; source-backed evidence passes as `claim_check_ready`.",
        "",
        f"3/ Demo: {public_demo_url} Repo: {repo_url} Playbook: {demo_pr_playbook_url}",
        "",
        "## Reply Bank",
        "",
        ...
        "## Short Post",
        "",
        f"Falsiflow turns AI eval, product metric, and R&D claims into auditable CI gates: claim -> evidence rows -> source provenance -> review bundle -> ready/blocked. Demo: {public_demo_url} Repo: {repo_url} Demo PR: {demo_pr_playbook_url}",
        "",
    ])
