...
def render_github_repo_profile(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        f"- Live downstream PR proof: {DOWNSTREAM_PR_URL}",
        f"- Downstream blocked CI run: {DOWNSTREAM_BLOCKED_RUN_URL}",
        f"- Downstream ready CI run: {DOWNSTREAM_READY_RUN_URL}",
...
def render_launch_posts(summary: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "Use these as drafts after the final public repository, hosted demo, live downstream PR, and release evidence are green.",
        "",
        "## Channel Checklist",
        "",
        ...
        "| --- | --- | --- | --- |",
        "| Hacker News / Show HN | Concise technical launch with a working demo. | Confirm PyPI status, demo URL, live downstream PR, release evidence, and responsible-use boundary. | Watch the first 60 minutes for install failures and unclear positioning questions. |",
        "| Reddit / community threads | Feedback from people running AI eval, product analytics, R&D review, or CI gates. | Choose one relevant community at a time and include the downstream PR proof plus demo PR playbook. | Turn repeated questions into docs or labeled GitHub issues within 24 hours. |",
        ...
        f"Live downstream PR: {DOWNSTREAM_PR_URL}",
        f"Blocked run: {DOWNSTREAM_BLOCKED_RUN_URL}",
        f"Ready run: {DOWNSTREAM_READY_RUN_URL}",
        f"Demo PR playbook: {demo_pr_playbook_url}",
        "",
        "The fastest demo is a real downstream PR that first fails because placeholder provenance backs an AI-eval claim, then turns green after source-backed rows and raw export evidence are added. It is not an ELN/LIMS, not a regulatory system, and not scientific proof. It is a reproducible audit layer for deciding whether supplied evidence is complete enough to move forward.",
        "",
        ...
        "",
        "The clean downstream PR shows the core behavior: placeholder evidence produces `claim_check_blocked`; source-backed evidence produces `claim_check_ready`; reviewers get JSON, Markdown, source manifests, dashboards, and a bundle verification report.",
        "",
        f"Live downstream PR: {DOWNSTREAM_PR_URL}",
        f"Blocked run: {DOWNSTREAM_BLOCKED_RUN_URL}",
        f"Ready run: {DOWNSTREAM_READY_RUN_URL}",
        ...
        "",
        "2/ The quickest demo is a downstream PR: placeholder AI-eval evidence fails as `claim_check_blocked`; source-backed evidence passes as `claim_check_ready`.",
        "",
        f"3/ Downstream PR: {DOWNSTREAM_PR_URL} Blocked: {DOWNSTREAM_BLOCKED_RUN_URL} Ready: {DOWNSTREAM_READY_RUN_URL}",
        "",
        f"4/ Demo: {public_demo_url} Repo: {repo_url} Playbook: {demo_pr_playbook_url}",
        "",
        "## Reply Bank",
        "",
        ...
        "## Short Post",
        "",
        f"Falsiflow turns AI eval, product metric, and R&D claims into auditable CI gates: claim -> evidence rows -> source provenance -> review bundle -> ready/blocked. Downstream PR: {DOWNSTREAM_PR_URL} Demo: {public_demo_url} Repo: {repo_url}",
        "",
    ])
