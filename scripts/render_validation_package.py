#!/usr/bin/env python3
"""Render a LIMINA validation package JSON into Markdown."""
...
DEFAULT_INPUT = ROOT / "data" / "zrc_nd_3p5k_guard_validation_package.json"
DEFAULT_SCHEMA = ROOT / "data" / "validation_package_schema.json"
DEFAULT_OUTPUT = ROOT / "reports" / "zrc_nd_3p5k_guard_validation_package.md"
...
def render(package: dict[str, Any]) -> str:
    lines = [
        ...
        f"**Parent technology:** `{package['parent_technology']}`",
        "",
        "## Objective",
        "",
        package["objective"],
    ...
    for item in package["procurement_specs"]:
        lines.append(
            f"| `{item['id']}` | {item['component']} | {item['required_spec']} | "
            f"{item['preferred_format']} | {item['reject_if']} | {refs_text(item)} |"
        )
    ...
    for item in package["test_articles"]:
        ...
        lines.append(f"| `{item['id']}` | `{variant}` | {item['role']} | {item['description']} |")
    ...
    for assay in package["assay_panel"]:
        lines.append(
            f"| `{assay['id']}` | {assay['readout']} | {assay['method_class']} | "
            f"{assay['sample_matrix']} | {assay['control_requirement']} | {refs_text(assay)} |"
        )
    ...
    for gate in package["acceptance_gates"]:
        lines.append(
            f"| `{gate['id']}` | {gate['criterion']} | {gate['rationale']} | "
            f"{gate['failure_response']} |"
        )
    ...
    for rule in package["decision_rules"]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render a LIMINA validation package.")
