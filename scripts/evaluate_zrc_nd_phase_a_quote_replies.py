#!/usr/bin/env python3
"""Score ZRC-ND Phase A quote replies without treating them as evidence."""
...
DEFAULT_TRACKER_CSV = ROOT / "data" / "zrc_nd_phase_a_quote_tracker.csv"
DEFAULT_RFQ = ROOT / "data" / "zrc_nd_phase_a_rfq_packet.json"
DEFAULT_REPLY_LOG = ROOT / "data" / "zrc_nd_phase_a_rfq_reply_log.json"
DEFAULT_JSON = ROOT / "data" / "zrc_nd_phase_a_quote_selection.json"
DEFAULT_REPORT = ROOT / "reports" / "zrc_nd_phase_a_quote_selection.md"
...
def score_row(row: dict[str, str], weights: dict[str, int], source_backed_ids: set[str]) -> dict[str, Any]:
    ...
    if not contact_sent:
    ...
    elif normalized_decision == "selected_for_phase_a":
        decision = "selected_for_phase_a"
    elif hard_failures:
        ...
        decision = "shortlist_for_phase_a_execution"
    else:
...
def build_selection(rows: list[dict[str, str]], rfq: dict[str, Any], reply_log: dict[str, Any]) -> dict[str, Any]:
    ...
    selected = [row for row in scored if row["selection_decision"] == "selected_for_phase_a"]
    shortlisted = [row for row in scored if row["selection_decision"] == "shortlist_for_phase_a_execution"]
    ...
    return {
        "status": status,
        "purpose": "Score RFQ replies for ZRC-ND Phase A execution selection without treating quotes as material evidence.",
        ...
        "scored_rows": ranked,
        "non_evidence_boundary": "Quote replies and vendor selection can authorize measurement execution only; they do not count as Phase A data or material suitability evidence.",
    }


def render_report(result: dict[str, Any]) -> str:
    lines = [
        "# ZRC-ND Phase A Quote Selection",
        "",
    ...
    lines.extend(["", "## Hard Required Fields", ""])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate ZRC-ND Phase A quote replies.")
...
def main() -> int:
    ...
    print(f"ZRC-ND Phase A quote selection: {result['status']}")
