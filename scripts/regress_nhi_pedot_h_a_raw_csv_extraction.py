#!/usr/bin/env python3
"""Regression checks for H-A raw CSV value extraction."""
...
from extract_nhi_pedot_h_a_raw_csv_values import extract_values


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_JSON = ROOT / "data" / "nhi_pedot_h_a_raw_csv_extraction_regression.json"
DEFAULT_REPORT = ROOT / "reports" / "nhi_pedot_h_a_raw_csv_extraction_regression.md"
...
def source_row(source_file: Path, target_field: str = "pH_initial", instrument_required: str = "true") -> dict[str, str]:
    return {
        "run_id": "run_a",
...
def run_case() -> dict:
    with tempfile.TemporaryDirectory(prefix="nhi_h_a_raw_csv_extraction_") as tmp:
        base = Path(tmp)
        source_dir = base / "source_files" / "full" / "h_a"
        raw = source_dir / "run_a_initial_ph.csv"
        raw.parent.mkdir(parents=True, exist_ok=True)
        source_values = base / "source_values.csv"

        no_raw_values = base / "no_raw_source_values.csv"
        write_csv(no_raw_values, ["run_id", "sample_event", "target_field", "unit", "instrument_required", "source_file"], [
            source_row(source_dir / "missing.csv")
        ])
        no_raw = extract_values(args_for(base, no_raw_values, base / "no_raw_out.csv"))

        write_csv(raw, ["value", "measured_at", "operator_or_agent", "instrument_id"], [{
            "value": "7.23",
            "measured_at": "2026-05-24T11:00:00+08:00",
            "operator_or_agent": "regression",
            "instrument_id": "ph-meter-1",
        }])
        write_csv(source_values, ["run_id", "sample_event", "target_field", "unit", "instrument_required", "source_file"], [
            source_row(raw)
        ])
        extracted = extract_values(args_for(base, source_values, base / "extracted.csv"))

        write_csv(raw, ["run_id", "sample_event", "target_field", "value", "measured_at", "operator_or_agent", "instrument_id"], [
            {
                "run_id": "run_a",
                "sample_event": "initial",
    ...
    checks = {
        "missing_raw_csv_is_not_synthetic": (
            no_raw["status"] == "h_a_raw_csv_value_extraction_no_raw_csv"
            and no_raw["summary"]["extracted_rows"] == 0
        ),
        "valid_raw_csv_extracts_one_row": (
            extracted["status"] == "h_a_raw_csv_value_extraction_ready"
            and extracted["summary"]["extracted_rows"] == 1
            and extracted["extracted_rows"][0]["value"] == "7.23"
        ),
        "placeholder_value_rejected": (
            rejected["status"] == "h_a_raw_csv_value_extraction_blocked"
...
def render_report(result: dict) -> str:
    lines = [
        "# NHI-PEDOT H-A Raw CSV Extraction Regression",
        "",
...
def main() -> int:
    ...
    print(f"NHI-PEDOT H-A raw CSV extraction regression: {result['status']}")
