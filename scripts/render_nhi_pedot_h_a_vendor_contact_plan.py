#!/usr/bin/env python3
"""Render official contact plan for sending NHI-PEDOT H-A RFQ bundles."""
...
DEFAULT_CSV = ROOT / "data" / "nhi_pedot_h_a_vendor_contact_plan.csv"
DEFAULT_JSON = ROOT / "data" / "nhi_pedot_h_a_vendor_contact_plan.json"
DEFAULT_REPORT = ROOT / "reports" / "nhi_pedot_h_a_vendor_contact_plan.md"
...
def build_plan(contacts: dict[str, Any], outbox: dict[str, Any], tracker: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        "status": "contact_plan_ready" if ready_or_sent == len(required_rows) and required_rows else "contact_plan_needs_attention",
        "purpose": "Official contact channels and send checklist for NHI-PEDOT H-A first-wave RFQ bundles plus standby secondary-wave options.",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render NHI-PEDOT H-A vendor contact plan.")
...
def main() -> int:
    ...
    print(f"H-A vendor contact plan: {result['status']}")
