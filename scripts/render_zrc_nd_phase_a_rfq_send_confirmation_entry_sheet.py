#!/usr/bin/env python3
"""Render a guarded entry sheet for non-EML ZRC-ND Phase A RFQ send confirmations."""
...
DEFAULT_DISPATCH_MANIFEST = ROOT / "data" / "zrc_nd_phase_a_rfq_dispatch_manifest.csv"
DEFAULT_SEND_LOG = ROOT / "data" / "zrc_nd_phase_a_rfq_send_log.csv"
DEFAULT_CSV = ROOT / "data" / "zrc_nd_phase_a_rfq_send_confirmation_entry_sheet.csv"
DEFAULT_JSON = ROOT / "data" / "zrc_nd_phase_a_rfq_send_confirmation_entry_sheet.json"
DEFAULT_REPORT = ROOT / "reports" / "zrc_nd_phase_a_rfq_send_confirmation_entry_sheet.md"
...
NON_EVIDENCE_BOUNDARY = (
    "Manual ZRC-ND Phase A RFQ send-confirmation entries document outreach logistics only. "
    "They are not quote replies, measurements, or material suitability evidence."
)
...
def build_sheet(args: argparse.Namespace) -> dict[str, Any]:
    _manifest_fields, manifest_rows = load_csv(args.dispatch_manifest)
    ...
    for manifest_row in manifest_rows:
        ...
        source_file = clean(prior.get("send_confirmation_source_file")) or clean(manifest_row.get("confirmation_source_file_to_save"))
        computed_hash = sha256(source_file)
        row = {
            "candidate_id": candidate_id,
            "vendor_name": clean(manifest_row.get("vendor_name")),
            "entry_status": "",
            "send_log_status": clean(log_row.get("send_status")) or clean(manifest_row.get("send_log_status")),
            ...
            "expected_bundle_sha256": clean(manifest_row.get("bundle_sha256_expected")) or clean(log_row.get("expected_bundle_sha256")),
            "sent_bundle_sha256_to_record": clean(prior.get("sent_bundle_sha256_to_record")) or clean(manifest_row.get("sent_bundle_sha256_to_record")),
            ...
            "send_method": clean(prior.get("send_method")) or clean(manifest_row.get("primary_send_method")),
            "recipient_or_form": clean(prior.get("recipient_or_form")) or clean(manifest_row.get("recipient_or_form")),
    ...
    if ready_rows:
        status = "zrc_phase_a_rfq_send_confirmation_entry_sheet_ready_to_apply"
    elif blocked_rows:
        status = "zrc_phase_a_rfq_send_confirmation_entry_sheet_needs_attention"
    elif already_sent_rows:
        status = "zrc_phase_a_rfq_send_confirmation_entry_sheet_sent_rows_present"
    else:
        status = "zrc_phase_a_rfq_send_confirmation_entry_sheet_waiting_for_confirmation_files"

    return {
        "status": status,
        "purpose": "Guarded manual-entry sheet for non-EML ZRC-ND Phase A RFQ send confirmations such as web-form confirmations, PDFs, screenshots, or saved pages.",
        "summary": {
            "entry_rows": len(rows),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render ZRC-ND Phase A RFQ send-confirmation entry sheet.")
...
def main() -> int:
    ...
    print(f"ZRC-ND Phase A RFQ send confirmation entry sheet: {result['status']}")
