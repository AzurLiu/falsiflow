#!/usr/bin/env python3
"""Render an actionable reply-intake pack for ZRC-ND Phase A RFQ replies."""
...
DEFAULT_CSV = ROOT / "data" / "zrc_nd_phase_a_rfq_reply_action_queue.csv"
DEFAULT_JSON = ROOT / "data" / "zrc_nd_phase_a_rfq_reply_action_pack.json"
DEFAULT_REPORT = ROOT / "reports" / "zrc_nd_phase_a_rfq_reply_action_pack.md"
DEFAULT_TEMPLATE_DIR = ROOT / "data" / "rfq_reply_review_templates" / "zrc_phase_a"
DEFAULT_REPLY_DIR = ROOT / "data" / "rfq_reply_files" / "zrc_phase_a"
...
def next_action_for(status: str) -> str:
    if status == "waiting_for_rfq_send":
        return "Send the ZRC-ND Phase A RFQ first, then save a real send confirmation and apply the send log."
    ...
    if status == "ready_for_reply_log_apply":
        return "Run apply_limina_rfq_reply_log.py, evaluate ZRC quote replies, then select or reject the provider."
    return "Review this row before proceeding."


def render_reply_template(row: dict[str, Any]) -> str:
    lines = [
        f"# {row['vendor_name']} ZRC-ND Phase A RFQ Reply Review Notes",
        "",
    ...
    lines.extend(["", "Hard required fields for execution selection:"])
...
def build_pack(args: argparse.Namespace) -> dict[str, Any]:
    ...
    status = "zrc_phase_a_rfq_reply_action_pack_ready"
    if not rows:
        status = "zrc_phase_a_rfq_reply_action_pack_no_rows"
    elif status_counts.get("waiting_for_rfq_send", 0) == len(rows):
        status = "zrc_phase_a_rfq_reply_action_pack_waiting_for_send"

    return {
        "status": status,
        "purpose": "Action queue for preserving source-backed ZRC-ND Phase A vendor replies and turning them into execution-selection inputs.",
        "summary": {
        ...
        "boundary": (
            ...
            "but only returned measurement files can become ZRC-ND material evidence."
        ),
    }
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Hard Required Selection Fields", ""])
    lines.extend(f"- `{field}`" for field in HARD_REQUIRED)
    lines.extend([
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render ZRC-ND Phase A RFQ reply action pack.")
...
def main() -> int:
    ...
    print(f"ZRC-ND Phase A RFQ reply action pack: {result['status']}")
    print(f"Action rows: {result['summary']['action_rows']}")
    print(f"Wrote {args.csv_out}")
    print(f"Wrote {args.json_out}")
    print(f"Wrote {args.report}")
    return 0 if result["status"] in {"zrc_phase_a_rfq_reply_action_pack_ready", "zrc_phase_a_rfq_reply_action_pack_waiting_for_send"} else 2
