#!/usr/bin/env python3
"""Apply reviewed non-EML ZRC-ND Phase A RFQ send confirmations into the send log."""
...
DEFAULT_SHEET = ROOT / "data" / "zrc_nd_phase_a_rfq_send_confirmation_entry_sheet.csv"
DEFAULT_SEND_LOG = ROOT / "data" / "zrc_nd_phase_a_rfq_send_log.csv"
DEFAULT_JSON = ROOT / "data" / "zrc_nd_phase_a_rfq_send_confirmation_entry_apply.json"
DEFAULT_REPORT = ROOT / "reports" / "zrc_nd_phase_a_rfq_send_confirmation_entry_apply.md"
...
NON_EVIDENCE_BOUNDARY = (
    "Applying ZRC-ND Phase A RFQ send-confirmation entries updates outreach logistics only. "
    "It does not create measurement evidence or advance material suitability gates."
)
...
def build_apply(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not apply_rows:
        status = "zrc_phase_a_rfq_send_confirmation_entry_apply_no_apply_rows"
    elif blocked:
        status = "zrc_phase_a_rfq_send_confirmation_entry_apply_blocked"
    else:
        status = "zrc_phase_a_rfq_send_confirmation_entry_apply_applied"
    return {
        "status": status,
        "purpose": "Apply reviewed non-EML ZRC-ND Phase A RFQ send confirmations into the send log without treating outreach as evidence.",
        "summary": {
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        "# ZRC-ND Phase A RFQ Send Confirmation Entry Apply",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Apply ZRC-ND Phase A RFQ send-confirmation entry sheet.")
...
def main() -> int:
    ...
    print(f"ZRC-ND Phase A RFQ send confirmation entry apply: {result['status']}")
    print(f"Applied: {result['summary']['applied_rows']}")
    print(f"Wrote {args.json_out}")
    print(f"Wrote {args.report}")
    return 0 if result["status"] != "zrc_phase_a_rfq_send_confirmation_entry_apply_blocked" else 2
