#!/usr/bin/env python3
"""Render a blank quote-response tracker for NHI-PEDOT H-A RFQ replies."""
...
DEFAULT_CSV = ROOT / "data" / "nhi_pedot_h_a_quote_tracker.csv"
DEFAULT_JSON = ROOT / "data" / "nhi_pedot_h_a_quote_tracker.json"
DEFAULT_REPORT = ROOT / "reports" / "nhi_pedot_h_a_quote_tracker.md"
...
def tracker_status(rows: list[dict[str, str]]) -> str:
    ...
    if any(row.get("decision") == "selected_for_h_a" for row in rows):
...
def build_tracker(rfq: dict[str, Any], rows: list[dict[str, str]]) -> dict[str, Any]:
    return {
        ...
        "purpose": "Track RFQ replies and decide which vendor/cooperator can generate claimable H-A measurement rows.",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render NHI-PEDOT H-A quote tracker.")
...
def main() -> int:
    ...
    print(f"H-A quote tracker: {result['status']}")
