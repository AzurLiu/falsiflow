#!/usr/bin/env python3
"""Render sendable RFQ outbox bundles for NHI-PEDOT H-A measurement vendors."""
...
DEFAULT_OUTBOX_DIR = ROOT / "data" / "nhi_pedot_h_a_rfq_outbox"
DEFAULT_JSON = ROOT / "data" / "nhi_pedot_h_a_rfq_outbox_manifest.json"
DEFAULT_REPORT = ROOT / "reports" / "nhi_pedot_h_a_rfq_outbox.md"
...
NON_EVIDENCE_BOUNDARY = (
    "RFQ emails, vendor capability replies, and quote packages are sourcing artifacts only. "
...
def write_bundle(
    ...
    candidate_id = safe_id(str(request.get("candidate_id", "vendor")))
    zip_path = package_dir / f"{candidate_id}_nhi_pedot_h_a_rfq_bundle.zip"
...
def render_outbox(rfq: dict[str, Any], delivery: dict[str, Any], outbox_dir: Path) -> dict[str, Any]:
    ...
    attachment_manifest_rows = attachment_rows(rfq, delivery)
    write_csv(attachment_manifest, ATTACHMENT_FIELDS, attachment_manifest_rows)

    missing = [row["path"] for row in attachment_manifest_rows if row["exists"] != "true"]
    ...
    return {
        "status": (
            ...
            if rfq.get("status") == "ready_to_send_rfq" and ready_count == len(rows) and not missing and not delivery_missing
            else "outbox_needs_attention"
        ),
        "purpose": "Vendor-specific RFQ emails and zip bundles for sending the real NHI-PEDOT H-A measurement request.",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render NHI-PEDOT H-A RFQ outbox bundles.")
...
def main() -> int:
    ...
    print(f"H-A RFQ outbox: {result['status']}")
